...
function InputRow({ label, sub, value, onChange, disabled, placeholder, readOnly, type = "text", min, max }) {
  return (
...
function SectionDivider({ label, danger }) {
  return (
...
export default function SettingsTab() {
  ...
  const fetchModels = useCallback(async () => {
    ...
    try {
      ...
      if (!d.reachable) {
        const mode = settings?.network_mode || "local";
        const hint = mode === "tailscale"
        ...
        if ((d.models || []).length === 0) {
          setModelsError("No models found. Pull one with: ollama pull llama3");
        }
      }
    } catch (e) {
  ...
  }, [settings?.ollama_api_url, settings?.network_mode]);
          ...
          <div className="card-body gap-6">
            ...
            <div className="space-y-4">
              <div className="flex items-start gap-2 text-xs text-base-content/50 bg-base-300/30 rounded-lg px-3 py-2.5 border border-base-300">
                <span className="text-base leading-none mt-0.5 shrink-0">🧠</span>
                <span>
                  Strategy explanations are powered by your locally-running Ollama instance.
                  {(settings.network_mode || "local") === "tailscale"
                    ? <> Use your Tailscale IP as the URL below.</>
                    : <> Supports <span className="font-mono">localhost</span>, LAN IPs, and Cloudflare Tunnel URLs.</>
                  }
                </span>
              </div>

              {/* URL input — only shown in Tailscale mode */}
              {(settings.network_mode || "local") === "tailscale" && (
                <InputRow
                  label="Ollama API URL"
                  sub="Your home machine's Tailscale IP, e.g. http://100.x.y.z:11434"
