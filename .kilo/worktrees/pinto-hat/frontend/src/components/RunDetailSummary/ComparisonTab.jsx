...
const ComparisonTab = ({ run }) => {
  ...
  // Initialize selected runs with current run if not already set
  React.useEffect(() => {
    if (selectedRuns.length === 0 && run) {
      setSelectedRuns([run]);
    }
  }, [run, selectedRuns.length]);
  ...
  const addRun = (runId) => {
    ...
    if (runToAdd && !selectedRuns.find(r => r.run_id === runId)) {
      setSelectedRuns([...selectedRuns, runToAdd]);
    }
    setShowAddRun(false);
  };
  ...
  const exportComparison = () => {
    ...
    const headers = ['Metric', ...selectedRuns.map(r => r.isCurrent ? 'Current Run' : `Run ${r.run_id}`)];
    const rows = comparisonMetrics.map(metric => [
      metric.label,
      ...selectedRuns.map(run => formatValue(run.metrics[metric.key], metric.format))
    ]);
      ...
      <div className="bg-base-200 border border-base-300 rounded-lg p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <h3 className="font-semibold text-base-content">Comparing {selectedRuns.length} Runs</h3>
            ...
            {!showAddRun ? (
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  {mockRuns
                    .filter(r => !selectedRuns.find(s => s.run_id === r.run_id))
                    .map(r => (
            ...
            <tbody className="divide-y divide-base-300">
              {comparisonMetrics.map((metric) => (
                <tr key={metric.key} className="hover:bg-base-300 transition">
                  ...
                  {selectedRuns.map((run, runIdx) => {
                    const currentValue = run.metrics[metric.key];
                    const previousValue = runIdx > 0 ? selectedRuns[runIdx - 1].metrics[metric.key] : null;
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-base-200 border border-base-300 rounded-lg p-5">
          ...
          <div className="text-lg font-bold text-success">
            {selectedRuns.reduce((best, current) => 
