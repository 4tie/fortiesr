...
def create_ollama_client_from_settings(
    ...
    try:
        ...
        logger.info(f"Creating OllamaClient from settings: base_url={base_url}, model={model}, timeout={timeout}")
        return OllamaClient(
            base_url=base_url,
            model=model,
            timeout=timeout,
            ...
            if suggested_tuple in tried_combinations:
                ...
                try:
                    retry_response = await asyncio.wait_for(
                        client.generate(retry_user_prompt, system_prompt=retry_system_prompt),
                        timeout=30.0
                    )
                    if retry_response:
                        retry_cleaned = clean_json_response(retry_response)
