...
from ...services.auto_quant_service import AutoQuantService
...
# Service instance for new streamlined pipeline
_service: AutoQuantService = None


def get_service() -> AutoQuantService:
    """Get or initialize AutoQuant service"""
    global _service
    if _service is None:
        _service = AutoQuantService(strategy_type="swing")
    return _service
...
async def get_status(run_id: str) -> dict:
    ...
    return _pl._state_snapshot(state)
...
async def cancel_pipeline(run_id: str) -> dict:
    if not _pl.request_cancel(run_id):
        raise HTTPException(status_code=404, detail=f"Pipeline run '{run_id}' not found.")
...
async def get_options(request: Request) -> AutoQuantOptions:
    ...
    services = request.app.state.services
    settings = services.settings_store.load()
    options_file = Path(settings.user_data_directory_path) / "auto_quant_options.json"
    
    if options_file.exists():
        try:
            data = json.loads(options_file.read_text(encoding="utf-8"))
            if isinstance(data.get("pair_universe"), list):
                data["pair_universe"] = ",".join(data["pair_universe"])
            return AutoQuantOptions(**data)
        except Exception:
            pass
    
    # Return default options if file doesn't exist or is invalid
    return AutoQuantOptions()
...
async def save_options(body: AutoQuantOptions, request: Request) -> dict:
    ...
    try:
        options_file.write_text(body.model_dump_json(indent=2), encoding="utf-8")
        return {"status": "success", "message": "Options saved"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to save options: {str(e)}")
