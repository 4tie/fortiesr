...
async def get_pairs(request: Request) -> dict:
    return _state_response(request.app.state.services)


@router.get("/search", summary="Search available pairs")
async def search_pairs(q: str, request: Request) -> dict:
...
async def toggle_favorite(body: dict, request: Request) -> dict:
    services = request.app.state.services
    try:
...
async def toggle_lock(body: dict, request: Request) -> dict:
    services = request.app.state.services
    try:
...
async def toggle_select(body: dict, request: Request) -> dict:
    services = request.app.state.services
    try:
...
async def randomize_pairs(request: Request, body: dict | None = None) -> dict:
    services = request.app.state.services
    try:
...
async def update_max_trades(body: dict, request: Request) -> dict:
    services = request.app.state.services
    try:
