#!/usr/bin/env python3
"""
Rapid Profitable Strategy Discovery Workflow

This script rapidly tests existing strategies via backtesting to find profitable ones
with at least 1 trade per day. It's much faster than the full AutoQuant pipeline.
...
class RapidStrategyDiscovery:
    """Rapid strategy discovery using direct backtesting."""
    ...
    def __init__(
        ...
        # Pairs to test (using existing config pairs)
        self.pairs = ["LTC/USDT", "XRP/USDT", "BNB/USDT", "LINK/USDT"]
        
        # Date ranges
        self.timeranges = [
            "20230101-20240101",  # 1 year
            "20230601-20240101",  # 6 months
            "20230901-20240101",  # 3 months
        ]
    ...
    def get_available_strategies(self) -> List[str]:
        ...
        for file in self.strategies_dir.glob("*.py"):
            # Skip backup files
            if "baseline_backup" not in file.name and "backup" not in file.name:
            ...
            if json_file.exists():
                try:
                    with open(json_file, 'r') as f:
                        ...
                        metrics.update(json_data.get("strategy", {}))
                except Exception:
                    pass
    ...
    def run_discovery_loop(self) -> Dict[str, Any]:
        ...
        print(f"Starting Rapid Profitable Strategy Discovery")
...
def main():
    """Main entry point."""
    discovery = RapidStrategyDiscovery(
        max_iterations=20,
        min_profit_pct=0.5,
    ...
    output_file = Path("rapid_strategy_discovery_results.json")
