...
- [x] 1.1 Replace `StatusBadge` inline `style` props with Tailwind utility class mapping object (`STATUS_CLASSES`). Replace `animate-spin` on HeroIcon with daisyUI `loading loading-spinner loading-xs` for running state.
- [x] 1.2 Apply state token system to card shell: use `border-l-2 border-l-primary` accent on running state; reduce pending border opacity; use `shadow-sm shadow-primary/10` only on running.
- [x] 1.3 Fix header layout: status badge right-aligned via `ml-auto`; step description always visible; duration shown as `(45s)` after badge for passed only.
- [x] 1.4 Auto-expand logic: use `defaultExpanded` prop (already exists). In the parent, pass `defaultExpanded={stage.status === 'running' || stage.status === 'failed' || stage.status === 'warning'}`. Inside the component, keep local `useState(defaultExpanded)` so the user can still manually collapse/expand after the initial render. Do NOT use a permanently forced prop that overrides user interaction.
- [x] 1.5 Replace section `border-t` dividers with `<div className="h-px bg-base-200/60" />` between sections in expanded area.
- [x] 1.6 Polish empty state: replace `ClockIcon` with step-number watermark in `text-3xl font-mono text-base-content/12` + `"Waiting to start"` text in `text-xs text-base-content/35`.
- [x] 1.7 Replace null metric values `"Not available"` with em-dash `—`.
- [x] 1.8 Add `transition-transform duration-200` to expand chevron; chevron rotates from `rotate-0` (collapsed) to `rotate-90` (expanded).
...
- **Never use `/8` Tailwind opacity modifiers** (e.g. `bg-primary/8`, `bg-warning/8`, `bg-error/8`). Use `/10` as the nearest supported step, or arbitrary syntax `bg-primary/[0.08]` when the precise low opacity matters.
- **Auto-expand is default-only, not forced.** Use `defaultExpanded` prop → `useState(defaultExpanded)`. The user must be able to collapse/expand manually after initial render.
