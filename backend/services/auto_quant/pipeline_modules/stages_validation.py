...
async def _stage_robustness_feature_injection(
    ...
    """Stage 3: Robustness & Feature Injection (Slippage/Fee Stress Testing).
    ...
    if not pairs_to_test:
        _rlog(run_id, 3, logging.WARNING, "Stage 3 | No selected_pairs available, skipping stress testing")
        state.stability_scores = {}
        _pass_stage(run_id, state, 3, "No pairs to test for stress testing", {"stability_scores": {}})
        return {"stability_scores": {}}

    _rlog(run_id, 3, logging.INFO,
          f"Stage 3 | Robustness & Feature Injection | strategy={strategy_name} | pairs={len(pairs_to_test)}")
    _emit(run_id, 3, "running", f"Running slippage/fee stress testing on {len(pairs_to_test)} pairs...", 55)

    # ── Sub-step 3.1: Losing windows analysis from Hyperopt backtest trades ─────────
    _rlog(run_id, 3, logging.INFO, "Stage 3 | Analyzing losing windows from Hyperopt backtest...")
    try:
        ...
        if hyperopt_result_data:
            ...
            _rlog(run_id, 3, logging.INFO,
                  f"Stage 3 | Trading window analysis: excluded_hours={trading_windows['excluded_hours']}, "
                  f"excluded_days={trading_windows['excluded_days']}")
        else:
            _rlog(run_id, 3, logging.WARNING, "Stage 3 | No Hyperopt backtest result found, skipping window analysis")
            state.excluded_time_windows = {"excluded_hours": [], "excluded_days": []}
    except Exception as exc:
        _rlog(run_id, 3, logging.WARNING, f"Stage 3 | Losing windows analysis failed: {exc}")
        state.excluded_time_windows = {"excluded_hours": [], "excluded_days": []}

    # ── Sub-step 3.2: Run 3 fee stress tests (1x, 2x, 3x) using temp configs ─────────
    ...
    try:
        for multiplier in fee_multipliers:
            _rlog(run_id, 3, logging.INFO, f"Stage 3 | Running stress test with {multiplier}x fees...")
            _emit(run_id, 3, "running", f"Running stress test with {multiplier}x fees...", 55 + (multiplier * 5))
            ...
            result_prefix = str(out_dir / f"stage3_stress_{multiplier}x")
            cmd = _backtest_cmd(
                state,
                strategy=strategy_name,
            ...
            if rc != 0:
                msg = _classify_subprocess_error(rc, stdout, f"Stage 3 (Stress Test {multiplier}x)")
                _rlog(run_id, 3, logging.ERROR, f"Stage 3 | FAIL | {msg}")
                _fail_stage(run_id, state, 3, msg)
                return None

            # Extract per-pair results
            result_data = _find_backtest_result(out_dir, f"stage3_stress_{multiplier}x", state.user_data_dir)
            per_pair = _extract_per_pair_results(result_data, strategy_name)
            ...
            _rlog(run_id, 3, logging.INFO,
                  f"Stage 3 | {multiplier}x stress test complete: {len(pair_profits)} pairs")
        ...
        for temp_config in temp_configs:
            try:
                ...
                _rlog(run_id, 3, logging.DEBUG, f"Stage 3 | Deleted temp config: {temp_config}")
            except Exception as exc:
                _rlog(run_id, 3, logging.WARNING, f"Stage 3 | Failed to delete temp config {temp_config}: {exc}")

    # ── Sub-step 3.3: Compute stability scores with division-by-zero guard ─────────
    _rlog(run_id, 3, logging.INFO, "Stage 3 | Computing stability scores...")
    state.stability_scores = {}
    ...
    for pair in pairs_to_test:
        ...
        # ── Sub-step 3.4: Emit WebSocket event for each pair ─────────────────────
        _emit(run_id, 3, "running",
        ...
        _rlog(run_id, 3, logging.DEBUG,
              f"Stage 3 | {pair}: stability={stability_score:.1f} "
              f"profits=[1x={profit_1x:.4f}, 2x={profit_2x:.4f}, 3x={profit_3x:.4f}]")

    _rlog(run_id, 3, logging.INFO,
          f"Stage 3 | Stability scores computed for {len(state.stability_scores)} pairs")

    # ── Sub-step 3.5: Safe feature injection (custom_stoploss + trading windows) ─────
    _rlog(run_id, 3, logging.INFO, "Stage 3 | Injecting custom_stoploss and trading window filters...")
    ...
    try:
        ...
        # Generate custom_stoploss method string (three-tier aggressive trailing stop)
...
'''
        ...
        # Inject custom_stoploss method after class definition
        ...
        if re.search(class_pattern, strategy_content):
            ...
            _rlog(run_id, 3, logging.DEBUG, "Stage 3 | Injected custom_stoploss method")
        else:
            _rlog(run_id, 3, logging.WARNING, f"Stage 3 | Could not find class definition for {strategy_name}")

        # Inject trading window filter attributes
        blocked_hours = state.excluded_time_windows.get("excluded_hours", [])
        blocked_days = state.excluded_time_windows.get("excluded_days", [])

        # Add blocked_hours and blocked_days as class attributes
        if blocked_hours or blocked_days:
            ...
            if class_line in strategy_content:
                ...
                strategy_content = '\n'.join(lines)
                _rlog(run_id, 3, logging.DEBUG,
                      f"Stage 3 | Injected trading window filters: hours={blocked_hours}, days={blocked_days}")

        # Write modified strategy back
        optimized_path.write_text(strategy_content, encoding="utf-8")
        _rlog(run_id, 3, logging.INFO, "Stage 3 | Feature injection successful")

    except Exception as exc:
        injection_success = False
        injection_error = str(exc)
        _rlog(run_id, 3, logging.ERROR,
              f"Stage 3 | Feature injection failed: {injection_error} | Continuing with unmodified strategy")
        _emit(run_id, 3, "running",
    ...
    _pass_stage(run_id, state, 3, "Robustness & Feature Injection complete", summary)
    return summary
