...
def retry_with_backoff(
    ...
    """Decorator for retry logic with custom backoff sequence.
    
    Args:
        max_retries: Maximum number of retry attempts
        backoff_sequence: List of delay seconds for each retry. Defaults to [10, 30, 40, 50]
    """
    if backoff_sequence is None:
        backoff_sequence = [10, 30, 40, 50]
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if attempt < max_retries:
                        ...
                        logger.warning(
                            f"Ollama API call failed (attempt {attempt + 1}/{max_retries + 1}): {e}. "
                    ...
                    logger.error(f"Ollama API call failed with non-retryable error: {e}")
                    raise
...
class CircuitBreaker:
    ...
    preventing calls to a failing service for a cooldown period.
    """
    ...
    def record_success(self) -> None:
        """Record a success and reset or close the circuit."""
        if self.state == "open":
    ...
    def should_allow_call(self) -> bool:
        ...
        if self.state == "open":
            ...
            if elapsed >= self.cooldown_seconds:
                ...
                logger.info("Circuit breaker entering half-open state for trial call.")
                return True
            return False
...
class OllamaClient:
    """Robust, non-blocking Ollama client with graceful fallbacks.
    
    This client handles all interactions with Ollama, including health checks
    and generation requests, with proper timeout handling and error recovery.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            timeout: Timeout in seconds for generate requests
            ...
            api_key: API key for Ollama Cloud API authentication
        """
    ...
    async def _get_session(self) -> aiohttp.ClientSession:
        """Get or create aiohttp session with enhanced error handling."""
        if self._session is None or self._session.closed:
            # Always set to None first if closed to ensure clean state
            if self._session is not None and self._session.closed:
                try:
                    await self._session.close()
                except Exception as e:
                    logger.warning(f"Error closing closed session: {e}")
                finally:
                    self._session = None
            ...
            try:
                self._session = aiohttp.ClientSession(
                    headers=headers,
                    timeout=aiohttp.ClientTimeout(total=self.timeout + 10)  # Extra buffer
                )
    ...
    async def check_health(self) -> bool:
        """Check if Ollama is reachable with timeout.
        ...
        try:
            ...
            await asyncio.wait_for(
                session.get(url, timeout=aiohttp.ClientTimeout(total=self.health_timeout)),
                timeout=self.health_timeout,
            )
            logger.debug("Ollama health check successful")
            return True
        except asyncio.TimeoutError:
            logger.warning(f"Ollama health check timed out after {self.health_timeout}s")
            return False
        except aiohttp.ClientError as e:
            logger.warning(f"Ollama health check failed: {e}")
            return False
        except Exception as e:
            logger.warning(f"Ollama health check unexpected error: {e}")
            return False

    @retry_with_backoff(max_retries=4, backoff_sequence=[10, 30, 40, 50])
    async def generate(
        self,
        prompt: str,
        ...
        Args:
            ...
            feature: Feature name for logging (e.g., "sensitivity_fix", "diagnostics")
        ...
        if not prompt or not isinstance(prompt, str):
            logger.error(f"Invalid prompt for feature '{feature}': must be non-empty string")
            return None
        
        if not self.model:
            logger.error(f"No model configured for Ollama client")
            return None
        ...
        try:
            ...
            if system_prompt:
                if not isinstance(system_prompt, str):
                    logger.error(f"Invalid system_prompt for feature '{feature}': must be string")
                    return None
            ...
            logger.debug(f"Sending request to Ollama for feature '{feature}' with model '{self.model}'")
            ...
            async with await asyncio.wait_for(
                ...
                if response.status != 200:
                    error_text = await response.text() if response.status else "No response body"
                    logger.warning(f"Ollama returned status {response.status}: {error_text}")
                    self._log_interaction(feature, prompt, system_prompt, None, duration_ms, False, f"HTTP {response.status}")
                    return None
                ...
                try:
                    ...
                    self._log_interaction(feature, prompt, system_prompt, None, duration_ms, False, "JSON parse error")
                    return None
                
                result = data.get("response", "")
                if not result:
                    logger.warning(f"Ollama returned empty response for feature '{feature}'")
                    self._log_interaction(feature, prompt, system_prompt, None, duration_ms, False, "Empty response")
                    return None
                
                if result:
                    logger.debug(f"Ollama generate successful, response length: {len(result)}")
            ...
            duration_ms = int((time.time() - start_time) * 1000)
            logger.warning(f"Ollama generate timed out after {self.timeout}s")
            self._log_interaction(feature, prompt, system_prompt, None, duration_ms, False, "Timeout")
            return None
        except aiohttp.ClientError as e:
            duration_ms = int((time.time() - start_time) * 1000)
            logger.warning(f"Ollama generate failed: {e}")
            self._log_interaction(feature, prompt, system_prompt, None, duration_ms, False, str(e))
            return None
        except json.JSONDecodeError as e:
            duration_ms = int((time.time() - start_time) * 1000)
            logger.warning(f"Ollama response JSON decode failed: {e}")
            self._log_interaction(feature, prompt, system_prompt, None, duration_ms, False, f"JSON decode: {e}")
            return None
        except Exception as e:
            duration_ms = int((time.time() - start_time) * 1000)
            logger.warning(f"Ollama generate unexpected error: {e}")
            self._log_interaction(feature, prompt, system_prompt, None, duration_ms, False, str(e))
            return None
    ...
    async def chat_with_tools(
        ...
        """Chat with Ollama with function calling support.
        ...
        Returns:
            Response object with content and optional tool_calls
        """
        import time
        start_time = time.time()
        ...
        try:
            ...
            async with await asyncio.wait_for(
                ...
                if response.status != 200:
                    logger.warning(f"Ollama chat returned status {response.status}")
                    self._log_interaction(feature, str(messages), None, None, duration_ms, False, f"HTTP {response.status}")
                    return type('Response', (), {'content': f"HTTP {response.status}", 'tool_calls': None})()
                ...
                class ChatResponse:
                    def __init__(self, data):
                        ...
                        if "tool_calls" in data.get("message", {}):
                            self.tool_calls = data["message"]["tool_calls"]
                
                response_obj = ChatResponse(data)
                
                if response_obj.content:
                    logger.debug(f"Ollama chat successful, response length: {len(response_obj.content)}")
                    self._log_interaction(feature, str(messages), None, response_obj.content, duration_ms, True, None)
                    return response_obj
                else:
                    logger.warning("Ollama chat returned empty response")
                    self._log_interaction(feature, str(messages), None, None, duration_ms, False, "Empty response")
                    return response_obj
                    
        except asyncio.TimeoutError:
            duration_ms = int((time.time() - start_time) * 1000)
            logger.warning(f"Ollama chat timed out after {self.timeout}s")
            self._log_interaction(feature, str(messages), None, None, duration_ms, False, "Timeout")
            return type('Response', (), {'content': "Timeout", 'tool_calls': None})()
        except aiohttp.ClientError as e:
            duration_ms = int((time.time() - start_time) * 1000)
            logger.warning(f"Ollama chat failed: {e}")
            self._log_interaction(feature, str(messages), None, None, duration_ms, False, str(e))
            return type('Response', (), {'content': str(e), 'tool_calls': None})()
        except json.JSONDecodeError as e:
            duration_ms = int((time.time() - start_time) * 1000)
            logger.warning(f"Ollama chat response JSON decode failed: {e}")
            self._log_interaction(feature, str(messages), None, None, duration_ms, False, f"JSON decode: {e}")
            return type('Response', (), {'content': f"JSON decode: {e}", 'tool_calls': None})()
        except Exception as e:
            duration_ms = int((time.time() - start_time) * 1000)
            logger.warning(f"Ollama chat unexpected error: {e}")
            self._log_interaction(feature, str(messages), None, None, duration_ms, False, str(e))
            return type('Response', (), {'content': str(e), 'tool_calls': None})()
    ...
    def _log_interaction(
        ...
        """Log ollama interaction to file if log_dir is configured."""
        if not self.log_dir:
            return
        ...
        try:
            ...
            log_file = log_path / "ollama_interactions.jsonl"
