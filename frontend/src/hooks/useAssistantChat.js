...
export function useAssistantChat({ contextOverrides = {}, mode: defaultMode = "analysis", onNewMessage } = {}) {
  ...
  const restoreSession = useCallback(async (sid) => {
    ...
    try {
      ...
      setMessages((prev) => {
        ...
        // FIX: Only add restored messages if we haven't already added them in this session
        // This prevents duplication when restoreSession is called after sendMessage
      ...
      while (true) {
        ...
        for (const part of parts) {
          ...
          if (event.type === "done") {
            ...
            if (onNewMessage) onNewMessage(assistantId);
            continue;
          }
  ...
  const sendMessage = useCallback(
    async (rawText, request = {}) => {
      ...
      if (onNewMessage) onNewMessage(userId);
