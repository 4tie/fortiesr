...
export default function GuidanceBubble({ contextOverrides = {}, onNavigate, activeTab }) {
  ...
  const handleClick = useCallback(() => {
    if (!dragStarted) {
      setIsPopupOpen((open) => {
        ...
        if (newOpen) {
          ...
          // Set last read message ID when closing
    ...
    setDragStarted(false);
  }, [dragStarted, position, latestMessageId]);
  ...
  const handleNewMessage = useCallback((messageId) => {
    setLatestMessageId(messageId);
    // If popup is closed and this is a new message (after last read), show notification
    if (!isPopupOpen && messageId !== lastReadMessageId) {
      setHasUnread(true);
    }
  }, [isPopupOpen, lastReadMessageId]);
