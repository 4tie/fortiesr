...
export default function useAutoQuantForm({ sharedState, sharedLoading, syncSharedState } = {}) {
  ...
  const syncSharedStateRef = useRef(syncSharedState);

  // Keep ref updated with latest syncSharedState function
  useEffect(() => {
  ...
  // Sync to sharedState
  useEffect(() => {
    if (!syncSharedStateRef.current || !hydrated.current) return;
    ...
    if (Object.keys(payload).length) syncSharedStateRef.current(payload);
  }, [form.strategy, form.timeframe, form.pair_universe, form.dry_run_wallet, form.max_open_trades]);
